...
if (mysqli_num_rows($resultSet) == 1) {
    ...
    while ($foundUser = mysqli_fetch_array($resultSet)) {
        ...
        $signatureImg = $harvestId.''.$water_id;
    }
...
if (isset($_POST['submit'])) {
    ...
    $gifter_signature = $_REQUEST['gifter_signature'];
    $gifter_name = $_REQUEST['gifter_name'];
    $current_time = date('Y-m-d');
    $imgData = addslashes(file_get_contents($_REQUEST['gifter_signature']));

    $query2 = "UPDATE harvests SET is_gifter_sign = 1, gifter_name='{$gifter_name}', gifter_signature = '{$imgData}', gifter_sign_date = '{$current_time}' WHERE id='{$id}' AND gifter_key='{$key}'";
    ...
    <div class="content row">
        ...
        <form name="sign_doc" action="" method="post" class="">
            <table style="width: 100%; color: red; text-align: left">
                <tr><td>&nbsp;</td></tr>
                <tr>
                    <td>Name of Gifter</td>
                    <td><input type="text" name="gifter_name" value="<?php echo $name?>" class="" required/></td>
                </tr>
                <tr><td>&nbsp;</td></tr>
                <tr>
                    <td>Signature</td>
                    <td>
                        <input type="text" name="gifter_signature_txt" id="signature" class="" value="" required />
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td><?php echo date('Y-m-d')?>
                        <input type="hidden" name="gifter_sign_date" value="<?php echo date('Y-m-d')?>" /></td>
                </tr>
                <tr>
                    <td colspan="2"><input type="checkbox" name="is_gifter_sign" value="1" required> Check box to sign document.</td>
                </tr>
                <tr>
            ...
            <input type="hidden" name="gifter_signature" id="gifter_signature" />
            <input type="hidden" name="gifter_key" id="gifter_key" value="<?php echo $key?>" />
        </form>
...
<script>
    $(document).ready(function(){
        ...
        signatureFont.change(function (){
            ...
            var dataString = 'font='+signatureFont.val()+'&signature='+signature.val()+'&type=gifter&signatureImg='+<?php echo $id?>;
