...
<section class="container" ng-app="">
  <?php include "_/components/php/header.php"; ?>
    <h2><i class="fas fa-hand-holding-water"></i> Board Request List</h2>

<?php
    $promoCode = $_SESSION['promoCode'];
    if($promoCode == "Admin") {$andPromo = "";} else {$andPromo = "AND users.promoCode = '{$promoCode}'";}
    ...
    $query .= "WHERE board_request.tlf_id = users.tlf_id  " .$andPromo . " ";
    $query .= "ORDER BY creationDateTime ASC";
    $resultSet = mysqli_query($connection, $query);
    confirm_query($resultSet, $connection);
    
    $fire_list = '<table class="table table-striped">
                        <thead>
                            <tr>
                              <th scope="col">#</th>
